...
public class LibraryService {
    ...
    // 책 이름으로 조회하는 서비스
    public List<Book> searchBooksByTile(String title) throws SQLException {
        // 입력값 유효성 검사
    ...
    public void borrowBook(int bookId, int studentId) throws SQLException {
        ...
        // borrowDAO 객체에게 협력 요청하고 borrows
        // 테이블에 insert 처리에 책임은 BorrowDAO 객체가 가진다
    ...
    public  void returnBook(int bookId, int studentId) throws  SQLException {
        if(bookId <= 0 || studentId <= 0){
            throw new SQLException("유효한 도서 ID와 학생 ID를 입력하세요");
        }
